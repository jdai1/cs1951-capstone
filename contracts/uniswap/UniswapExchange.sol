...
import "../utils/InverseTrigonometry.sol";
...
contract UniswapExchange is ERC20 {
 ...
 /**
  ...
  /** V --> X
   * @notice Calculate the value needed to reach stable state.
   * @dev Uses formula: phi = sqrt(c * (1 - v) / v)
   * @dev Where x = ETH reserve, y = Token reserve, v = target value
   * @return The value that would create a stable state for the current reserves scaled by 1e18 for precision (0 < V < 1e18)
   * 
   * tl;dr given a value v, what is the eth reserves that would create a stable state?
   */
  function valueToStableState(uint256 v, uint256 x, uint256 y) public view returns (uint256) {
    uint256 c = x.mul(y);
    require(x > 0 && y > 0, "INVALID_RESERVES");
    require(v > 0 && v < 1e18, "INVALID_V");

    // Calculate: phi = sqrt(c * (1 - v) / v)
    uint256 numerator = c.mul(1e18 - v);
    uint256 denominator = v;
    
    return InverseTrigonometry.sqrt(numerator / denominator);
  }

  function valueToStableStateWrapper(uint256 v) public view returns (uint256) {
    return valueToStableState(v, address(this).balance, token.balanceOf(address(this)));
  }

  /** X --> V
   * @notice Calculate the stable state value for current reserves.
   * @dev Uses formula: psi = c / (eth_reserve^2 + c)
   * @dev Where c = current_eth_reserve * current_token_reserve (the invariant)
   * @dev And x = arbitrary ETH reserve (parameter)
   * @param x X reserves of AMM
   * @param y Y reserves of AMM
   * @param eth_reserve hypothetical new X reserve of AMM
   * @return The stable state value scaled by 1e18 for precision (0 < V < 1e18)
   * 
   * tl;dr assume the current eth reserve is eth_reserve, and the AMM is in a stable state. What is the value of V?
   */
  function stableStateToValue(uint256 x, uint256 y, uint256 eth_reserve) public view returns (uint256) {
    require(eth_reserve > 0, "INVALID_ETH_RESERVE");
    require(x > 0 && y > 0, "INVALID_RESERVES");
    
    uint256 c = x.mul(y);
    
    // Calculate: psi = c / (eth_reserve^2 + c), scaled by 1e18
    uint256 numerator = c.mul(1e18);
    uint256 denominator = eth_reserve.mul(eth_reserve).add(c);
    return numerator / denominator;
  }

  function stableStateToValueWrapper(uint256 eth_reserve) public view returns (uint256) {
    return stableStateToValue(address(this).balance, token.balanceOf(address(this)), eth_reserve);
  }


  // /***********************************|
  // |     Divergence Loss Functions     |
  // |__________________________________*/

  /**
  * @notice Calculate the divergence loss for an ETH to Token trade.
  * @dev Uses formula: ψ(x') * x + (1 - ψ(x')) * f(x) - (ψ(x') * x' + (1 - ψ(x')) * f(x'))
  * @dev Where f(x) = c/x (token amount for given ETH reserve using constant product)
  * @param eth_sold Amount of ETH sold.
  * @return Divergence loss of proposed trade (scaled by 1e18).
  */
  function getEthToTokenDivergenceLoss(uint256 x, uint256 y, uint256 eth_sold) public view returns (uint256) {
    require(eth_sold > 0, "INVALID_ETH_SOLD");

    uint256 c = x.mul(y);
    uint256 x_prime = x.add(eth_sold);
    
    // Get f(x) and f(x')
    uint256 f_x = c.div(x);
    uint256 f_x_prime = c.div(x_prime);
    
    require(f_x == y, "INVALID_F_X");
    require(x.mul(y) == c, "INVALID_C");
    
    // Get ψ(x') - stable state value for new reserve
    uint256 v = stableStateToValue(x, y, x_prime);

    // Calculate: ψ(x') * x + (1 - ψ(x')) * f(x)
    uint256 term1 = ((v.mul(x)) + (1e18 - v).mul(f_x)) / 1e18;
    
    // Calculate: ψ(x') * x' + (1 - ψ(x')) * f(x')
    uint256 term2 = (v.mul(x_prime) + ((1e18 - v).mul(f_x_prime))) / 1e18;

    return term1 - term2;
  }

  function getEthToTokenDivergenceLossWrapper(uint256 eth_sold) public view returns (uint256) {
    return getEthToTokenDivergenceLoss(address(this).balance, token.balanceOf(address(this)), eth_sold);
  }

  /**
  * @notice Calculate the divergence loss for a Token to ETH trade.
  * @dev Uses formula: ψ(x') * y + (1 - ψ(x')) * f(y) - (ψ(x') * y' + (1 - ψ(x')) * f(y'))
  * @dev Where f(y) = c/y (ETH amount for given token reserve using constant product)
  * @dev And x' is calculated from constant product: x * y = x' * y'
  * @param x Current ETH reserve
  * @param y Current token reserve
  * @param token_sold Amount of tokens sold.
  * @return Divergence loss of proposed trade (scaled by 1e18).
  */
  function getTokenToEthDivergenceLoss(uint256 x, uint256 y, uint256 token_sold) public view returns (uint256) {
    require(token_sold > 0, "INVALID_TOKEN_SOLD");

    uint256 c = x.mul(y);
    uint256 y_prime = y.add(token_sold);
    uint256 x_prime = c.div(y_prime);
    ...
    // Calculate: ψ(x') * y + (1 - ψ(x')) * f(y)
    uint256 term1 = ((v.mul(x)) + (1e18 - v).mul(y)) / 1e18;
    
    // Calculate: ψ(x') * y' + (1 - ψ(x')) * f(y')
    uint256 term2 = (v.mul(x_prime) + ((1e18 - v).mul(y_prime))) / 1e18;

    return term1 - term2;
  }

  function getTokenToEthDivergenceLossWrapper(uint256 token_sold) public view returns (uint256) {
    return getTokenToEthDivergenceLoss(address(this).balance, token.balanceOf(address(this)), token_sold);
  }

  
  // /***********************************|
  // |     Linear Slippage Functions     |
  // |__________________________________*/

  function getEthToTokenLinearSlippage(uint256 x, uint256 y, uint256 eth_sold) public view returns (uint256) {
    require(eth_sold > 0, "INVALID_ETH_SOLD");

    uint256 c = x.mul(y);
    uint256 x_prime = x.add(eth_sold);
    uint256 y_prime = c.div(x_prime);

    uint256 v = stableStateToValue(x, y, x);
    uint256 v_prime = stableStateToValue(x, y, x_prime);

    uint256 factor1 = (1e18 - v_prime) * 1e18 / (1e18 - v);

    uint256 term1 = ((v * x_prime) + ((1e18 - v) * y_prime)) / 1e18;
    uint256 term2 = ((v * x) + ((1e18 - v) * y)) / 1e18;

    return factor1 * (term1 - term2) / 1e18;
  }

  function getEthToTokenLinearSlippageWrapper(uint256 eth_sold) public view returns (uint256) {
    return getEthToTokenLinearSlippage(address(this).balance, token.balanceOf(address(this)), eth_sold);
  }

  function getTokenToEthLinearSlippage(uint256 x, uint256 y, uint256 token_sold) public view returns (uint256) {
    require(token_sold > 0, "INVALID_TOKEN_SOLD");
  ...
  function getTokenToEthLinearSlippageWrapper(uint256 token_sold) public view returns (uint256) {
    return getTokenToEthLinearSlippage(address(this).balance, token.balanceOf(address(this)), token_sold);
  }
  ...
  function getConstantProduct(uint256 x, uint256 y) public view returns (uint256) {
    return x.mul(y);
  }

  /**
   * @notice Calculate arcsine of a value
   * @dev Uses InverseTrigonometry library for high-precision arcsine calculation
   * @param x Value with 18 decimal precision, must be in range [-1e18, 1e18]
   * @return arcsine result with 18 decimal precision
   */
  function arcsin(int256 x) public pure returns (int256) {
    return InverseTrigonometry.arcsin(x);
  }

  /**
   * @notice Calculate arctangent of a value
   * @dev Uses InverseTrigonometry library for high-precision arctangent calculation
   * @param x Input value with 18 decimal precision
   * @return arctangent result with 18 decimal precision
   */
  function arctan(int256 x) public pure returns (int256) {
    return InverseTrigonometry.atan(x);
  }

  // /***********************************|
  // |     Angular Slippage Functions    |
  // |__________________________________*/

  function getEthToTokenAngularSlippage(uint256 x, uint256 y, uint256 eth_sold) public view returns (int256) {
    require(eth_sold > 0, "INVALID_ETH_SOLD");
    ...
    int256 numerator = int256(v) - int256(v_prime);
    int256 denominator = int256((v * v_prime + (1e18 - v) * (1e18 - v_prime)) / 1e18);

    return arctan(numerator * 1e18 / denominator);
  }

  function getTokenToEthAngularSlippage(uint256 x, uint256 y, uint256 token_sold) public view returns (int256) {
    require(token_sold > 0, "INVALID_TOKEN_SOLD");
  ...
  function getEthToTokenLoad(uint256 x, uint256 y, uint256 eth_sold) public view returns (uint256) {
    require(eth_sold > 0, "INVALID_ETH_SOLD");

    return getEthToTokenDivergenceLoss(x, y, eth_sold) * getEthToTokenLinearSlippage(x, y, eth_sold) / 1e18;
  }


  function getTokenToEthLoad(uint256 x, uint256 y, uint256 token_sold) public view returns (uint256) {
    require(token_sold > 0, "INVALID_ETH_SOLD");

    return getTokenToEthDivergenceLoss(x, y, token_sold) * getTokenToEthLinearSlippage(x, y, token_sold) / 1e18;
  }
}
